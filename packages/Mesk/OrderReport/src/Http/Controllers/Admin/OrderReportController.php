...
namespace Mesk\OrderReport\Http\Controllers\Admin;
...
use Mesk\OrderReport\Mail\ShipmentUpdateNotification;
...
use Webkul\Admin\DataGrids\OrderReportDataGrid;
...
class OrderReportController extends Controller
{
   ...
   // ProductAttributeValueRepository $productAttributeValueRepository,ProductInventoryRepository $productInventoryRepository,AttributeRepository $attributeRepository
    ...
    public function index()
    {
		...
		$status=array("" => "Please Select","processing" => "Processing","completed" => "Completed","canceled" => "Canceled","closed" => "Closed","pending" => "Pending","pending_payment" => "Pending Payment","fraud" => "Fraud");
  		if (request()->ajax()) {
			return app(OrderReportDataGrid::class)->toJson();
		}
        ...
        if (in_array($file->getClientOriginalExtension(), $valid_extension)) {
            ...
            if ($uploadedFile) {
                ...
                if(count($errData)==0){
                    ...
                    session()->flash('success', trans('orderreport::app.admin.bulk-upload.messages.imported'));
                    return redirect()->route('admin.orderreport.index');
					
                }
				session()->flash('error', trans('orderreport::app.admin.bulk-upload.messages.cant-able-to-import'));
                return view($this->_config['view'], compact('errData'));  
            }
        } else {
            session()->flash('error', trans('orderreport::app.admin.bulk-upload.messages.file-format-error'));
            return redirect()->route('admin.orderreport.index');
        }
