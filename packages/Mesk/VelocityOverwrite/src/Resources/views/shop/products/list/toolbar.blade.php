    ...
    <script type="text/x-template" id="toolbar-template">
        <div class="toolbar-wrapper" v-if='!isMobile()'>
            <div class="view-mode">
                ...
                <div class="rango-view-grid-container display-none {{ $viewOption === 'grid' ? 'active' : '' }}">
                    <a href="{{ $toolbarHelper->getModeUrl('grid') }}" class="grid-view unset" aria-label="Grid">
                        <span class="rango-view-grid fs24"></span>
                    </a>
                </div>
                <div class="rango-view-list-container display-none {{ $viewOption === 'list' ? 'active' : '' }}" aria-label="List">
                    <a
			...
			<div class="items-count">
					...
					@if(!empty($_GET['limit']))
						@foreach ($toolbarHelper->getAvailableLimits() as $limit)
							@if($toolbarHelper->isLimitCurrent($limit))
								{{ $limit }} Iteam(s)
							@endif
						@endforeach
					@else
			...
			<div>
			<img loading="lazy" alt="wave" src="{{ asset('/themes/velocity/assets/images/wave-2.png') }}">
			</div>
