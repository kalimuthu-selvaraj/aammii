...
<?php
$helper = app('Webkul\Velocity\Helpers\Helper');
if(isset($product) && isset($product->product_id)){
    ...
    $checkCateg=$category;
    do {
        $helper->makeBreadCrumb($checkCateg);
        $checkCateg=$checkCateg->parent;
    }while(isset($checkCateg->parent));
...
<div class="breadcums container"><span class="text-black fs16 pr-20"><a class="text-black" href="{{ route('shop.home.index') }}">Home</a></span><?php foreach($breadCrumbs as $va=>$key){?>
        <a href="{{url($key['url'])}}"><span class="theme-color opacity-5">.</span><span class="fs16 pr-20 theme-color">{{$key['name']}}</span></a> 
        <?php } ?></div>
