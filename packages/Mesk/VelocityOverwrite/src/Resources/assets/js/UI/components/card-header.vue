<template>
    ...
    <div :class="`row mt-50 mobile-mt-50 mb15 col-12 ${rowClass}`">
        <div class="col-md-10 no-padding" v-if="tabs || viewAll || scrollable">
            <h2 class="fs28 fw6 mobile-fs20">{{ headerHeading }} <img loading="lazy" class="mobile-width-75 width-70" alt="wave" :src="`${$root.baseUrl}/themes/velocity/assets/images/single-lg.png`"></h2>
        </div>
		
		<div class="col-md-10 no-padding" v-else >
            <h2 class="fs28 fw6 mobile-fs20">{{ headerHeading }} <img loading="lazy" class="mobile-width-75 width-70" alt="wave" :src="`${$root.baseUrl}/themes/velocity/assets/images/single-lg.png`"></h2>
        </div>
            ...
            <div class="row justify-content-end text-right">
                ...
                <template v-if="tabs">
                    <div
                        ...
                        <h2 class="fs16 fw6 cursor-pointer tab view-btn mobile-view-all" :class="index == 0 ? 'active' : ''">{{ tab }}</h2>
                    </div>
                </template>
                ...
                <template v-if="(! (viewAll == 'false' || viewAll == '')) && viewAll">
                    <div>
                        <a :href="viewAll" :title="`View all ${headerHeading} products`" class="remove-decoration link-color">
                            <h2 class="fs16 fw6 cursor-pointer tab view-btn mobile-view-all">{{ __('home.view-all') }}</h2>
                        </a>
                    </div>
                </template>
