...
@section('page_title')
    {{ __('mobikul::app.mobikul.category.edit-category') }}
@stop

@section('content')
    <div class="content">
        <form method="POST" action="{{ route('mobikul.featured-category.update') }}" @submit.prevent="onSubmit" enctype="multipart/form-data">
            ...
            <div class="page-content">
                <div class="form-container">
                    ...
                    <div class="control-group" :class="[errors.has('image') ? 'has-error' : '']">
                        ...
                        <image-wrapper :button-label="'{{ __('mobikul::app.mobikul.notification.notification-image') }}'" input-name="image" :multiple="false" :images='"{{ $data->image_url }}"'></image-wrapper>
                    ...
                    <div class="control-group" :class="[errors.has('sort_order') ? 'has-error' : '']">
                        ...
                        <input type="text" valid="" id="sort_order" class="control" name="sort_order" v-validate="'required|numeric'"  value="{{ $data['sort_order'] ?? old('sort_order') }}" data-vv-as="&quot;{{ __('mobikul::app.mobikul.banner-image.sort-order') }}&quot;" />
                        ...
                        <select  v-validate="'required'" id="channels" class="control" name="channels[]" multiple="multiple" data-vv-as="&quot;{{ __('mobikul::app.mobikul.notification.store-view') }}&quot;">
                            @foreach ($channels as $channel)
                                <option value="{{ $channel->id }}"
                                    @if ( in_array($channel->id, $data->featuredCategoryChannelsArray())) selected @endif >
                                    {{ $channel->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="control-group" id="category">
                        ...
                        <tree-view value-field="id" name-field="category_id" input-type="radio" items='@json($categories)' value='@json($data['category_id'])'></tree-view>
                    </div>
                </div>
            </div>
        </form>
