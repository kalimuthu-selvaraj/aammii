...
@section('page_title')
    {{ __('mobikul::app.mobikul.carousel.new-carousel-image') }}
@stop

@section('content')
    <div class="content">
        <form method="POST" action="{{ route('mobikul.carousel.image.store') }}" @submit.prevent="onSubmit" enctype="multipart/form-data">
            ...
            <div class="page-content">
                ...
                <div class="form-container">
                    ...
                    <div class="control-group" :class="[errors.has('image') ? 'has-error' : '']">
                        <label for="image" class="required">{{ __('mobikul::app.mobikul.carousel.images') }}</label>

                        <image-wrapper :button-label="'{{ __('mobikul::app.mobikul.carousel.images') }}'" input-name="image"  name="image" data-vv-as="&quot;{{ __('mobikul::app.mobikul.carousel.image') }}&quot;" :multiple="false"></image-wrapper>
                    ...
                    <div class="control-group" :class="[errors.has('status') ? 'has-error' : '']">
                        ...
                        <select class="control" name="status" v-validate="'required'" data-vv-as="&quot;{{ __('mobikul::app.mobikul.carousel.status') }}&quot;">
                            <option value="1" {{ old('status') == 1 ? 'selected' : '' }}>{{ __('mobikul::app.mobikul.notification.status.enabled') }}</option>
                            <option value="0" {{ old('status') == 0 ? 'selected' : '' }}>{{ __('mobikul::app.mobikul.notification.status.disabled') }}</option>
                        </select>
