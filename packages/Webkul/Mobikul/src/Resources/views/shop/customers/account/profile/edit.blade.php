...
@section('page-detail-wrapper')
    ...
    <form
        ...
        action="{{ route('mobikul-api.customer.profile.edit') }}"
        ...
        <div class="account-table-content">
            ...
            <div class="{!! $errors->has('profile_pic.*') ? 'has-error' : '' !!}">
                <label>{{ __('mobikul-api::app.customer.account.profile.profile-pic') }}</label>

                <image-wrapper :button-label="'{{ __('mobikul-api::app.customer.account.profile.profile-pic') }}'" input-name="profile_pic" :multiple="false"  :images='"{{ $customer->profile_pic_url }}"'></image-wrapper>

                <span class="control-error" v-if="{!! $errors->has('profile_pic.*') !!}">
                    @foreach ($errors->get('profile_pic.*') as $key => $message)
                        @php echo str_replace($key, 'Profile Pic', $message[0]); @endphp
                    @endforeach
                </span>
