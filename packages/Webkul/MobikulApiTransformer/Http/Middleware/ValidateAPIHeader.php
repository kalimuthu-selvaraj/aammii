...
namespace Webkul\MobikulApiTransformer\Http\Middleware;
...
class ValidateAPIHeader
{
    ...
    */
    public function handle($request, Closure $next)
    {
        ...
        $channelId = request()->input('storeId') ?? core()->getDefaultChannel()->id;
        ...
        if (! $this->validateConfigHeader())
        {
            return response()->json([
                'success'   => false,
                'message'   => trans('mobikul-api::app.api.auth.invalid-auth'),
            ], 401);
        }
    ...
    public function validateConfigHeader()
    {
        ...
        $config_username = core()->getConfigData('mobikul.mobikul.basicinformation.username');
        $config_password = core()->getConfigData('mobikul.mobikul.basicinformation.password');
