...
class OrderReportExportDataGrid extends DataGrid
{
    ...
    public function prepareQueryBuilder()
    {
        ...
        $queryBuilder = DB::table('orders')
            ...
            ->addSelect('orders.id as ID', 'orders.created_at as DATE',DB::raw('CONCAT(' . DB::getTablePrefix() . 'orders.customer_first_name, " ", ' . DB::getTablePrefix() . 'orders.customer_last_name) as CUSTOMER_NAME'),'customers.phone as PHONE_NO','orders.customer_email as EMAIL','order_address_billing.address1 as ADDRESS','order_address_billing.postcode as PINCODE',
'order_address_billing.city as CITY','order_address_billing.state as STATE','order_address_billing.country as COUNTRY',
'order_payment.method as PAYMENT','orders.grand_total as AMOUNT','orders.shipping_amount as SHIPPING_CHARGES','orders.shipping_method as SHIPPING_METHOD', 'channel_name as CHANNEL_NAME',DB::raw(" '' as HSNCODE"),DB::raw(" '' as CATEGORY"),DB::raw(" '' as BARCODE"),'order_items.name as PRODUCT_NAME','order_items.qty_ordered as QTY','order_items.price as MRP','order_items.total as ORDER_AMOUNT',DB::raw(" order_items.total - order_items.tax_amount as PRICE_WITH_OUT_TAX "),'order_items.tax_amount as TAX_AMOUNT','order_items.discount_amount as DISCOUNT',DB::raw(" order_items.total - order_items.discount_amount  as TOTAL"), 'orders.status as STATUS','shipments.carrier_title as CARRIER_TITLE','shipments.track_number as TRACKING_NUMBER','order_items.product_id as PRODUCT_ID');
