...
class Cart
{
     ...
     * @return bool|void|Exception
     */
    public function updateMobileItems($data)
    {
        $cart = $this->getCart();
        foreach ($data['qty'] as $itemId => $quantity) {
            $item = $this->cartItemRepository->findOneWhere(array('product_id' => $itemId, 'cart_id' => $cart->id));
    ...
    public function collectTotals(): void
    {
		...
		//new added//
		$cart->grand_total = $cart->grand_total - $cart->tax_total;
        $cart->base_grand_total = $cart->base_grand_total - $cart->base_tax_total;
		//new added//
